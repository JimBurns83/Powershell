...
#$HostList = @{}
#$HostList.add("ComputerName",".")
$out = @()

foreach($object in $HostList) {
    
    $os = Get-WmiObject -computername $object.("ComputerName") -class win32_operatingsystem
    $vol = Get-WmiObject -computername $object.("ComputerName") -class Win32_Volume | select -First 1
    ...
    $sqlversion = gwmi -ComputerName $object.("ComputerName") -query "select name, version from win32_product where name like '%SQL%'" | select @{n="nver";e={$_.name + " " + $_.version}}
    $DeviceInfo= @{}
    ...
    $DeviceInfo.add("SQL", ($sqlversion.nver -join (", ")))
    ...
    $out += New-Object PSObject -Property $DeviceInfo | Select-Object `
              ...
              "Free Capacity","MAC Address","IP Address","Subnet", "SQL"
